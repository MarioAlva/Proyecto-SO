...
int PonJugadors(Partida *s, int socket)
{
	if (s->num<100)
	{
		s->socket[s->num]=socket;
		s->num++;
		return socket;
	}
	else
	...
	while (terminar == 0)
	{
		...
		if (codigo==8)//Respuesta del invitado
		{
			...
			if (r==0)
			{
				...
				int p= PonJugadors(&lista_partidas, isocket);
				int c= PonJugadors(&lista_partidas, sock_conn);
				printf("%d/%d\n",p, c);
		...
		{
			int random = atoi(lista_servicios.servicios[0].orden);
			int numero = rand () % 100;
			//if (r==0)
			//{
				int k = 0;
				strcpy(respuesta, "8/");//ha aceptado la invitacion
				sprintf(respuesta, "%s%d",respuesta, numero);
				printf("%s\n",respuesta);
				printf("%d\n", lista_partidas.num);
				for(k=0; k<lista_partidas.num; k++)
				{
					printf("%d\n", lista_partidas.socket[k]);
					write(lista_partidas.socket[k],respuesta, strlen(respuesta));
				}
			//else
			//{
				//strcpy(respuesta, "8/1");//ha rechazado la invitacion
				//printf ("%d\n", isocket);
				//write(isocket , respuesta,strlen(respuesta));
			//}
		}
